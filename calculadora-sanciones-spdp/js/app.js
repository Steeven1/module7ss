  ...
  input.addEventListener("input", () => {
    ...
    if (hadInvalidChars) {
      ...
      errorBox.textContent =
        "Solo se permiten números y punto decimal. No uses letras ni caracteres especiales.";
    }
...
form.addEventListener("submit", (event) => {
  ...
  const rdmMaxFineUsd = getRdmMaximumFineUsd(
  ...
  const institutionLabel =
    values.institutionType === "private"
      ? "Privada con fines de lucro (MPRIV-1)"
      : "Pública (MPUB-1)";
  const infractionLabel = values.infractionType === "grave" ? "Grave" : "Leve";
  const baseMetricLabel =
    values.institutionType === "public"
      ? "SBU (Salario Básico Unificado) en USD"
      : "VDN (Volumen de Negocio) en USD";
...
function applyRdmBySelection() {
  ...
  activeRangeInfo.textContent = buildRangeInfoText(
    institutionType,
    infractionType,
    rdmRange,
  );
...
function validate(values) {
  ...
  if (invalidInputNames.size > 0) {
    errors.push(
      "Se detectaron letras o caracteres especiales en campos numéricos. Corrige esos valores.",
    );
  }
  ...
  if (
    values.institutionType === "public" &&
    values.baseMetric !== FIXED_ECUADOR_SBU
  ) {
    errors.push(
      "Para sector público, el SBU es informativo y fijo en USD 482.00.",
    );
  }
  if (!isFiniteNumber(values.rdmMin) || !isFiniteNumber(values.rdmMax)) {
    errors.push(
      "No se pudo fijar el RDM. Selecciona institución e infracción.",
    );
  }
  ...
  if (
    !isFiniteNumber(values.rerCoef) ||
    values.rerCoef < 0 ||
    values.rerCoef > 1
  ) {
    errors.push(
      "El coeficiente de reincidencia (RER) debe ser un número entre 0 y 1.",
    );
  }
...
function validatePertTriple(triple, name, errors) {
  if (
    !isFiniteNumber(triple.min) ||
    !isFiniteNumber(triple.prob) ||
    !isFiniteNumber(triple.max)
  ) {
  ...
  if (!(triple.min <= triple.prob && triple.prob <= triple.max)) {
    errors.push(
      `${name}: se recomienda el orden Mínimo <= Probable <= Máximo.`,
    );
  }
}
...
function updateCalculationAvailability() {
  calculateBtn.disabled = !(
    getSelectedValue("institutionType") && getSelectedValue("infractionType")
  );
}
